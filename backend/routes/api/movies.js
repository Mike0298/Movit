...
router.post("/", async (req, res) => {
    ...
    if (writer)
        movieFields.writer = writer.split(",").map((writer) => writer.trim());
...
router.delete("/:movie_id", auth, async (req, res) => {
    ...
    if (user.accountType === "member")
        return res.status(401).json({
            error: "Insufficient permission, only admin and moderator can modify a movie",
        });
    ...
    try {
        ...
        for (let profile of profilesWithMovie) {
            const profileToRemoveFrom = await Profile.findOne({
                user: profile.user,
            });
    ...
    try {
        ...
        if (
            profile.likedMovies.filter(
                (likedMovie) =>
                    likedMovie.movie.toString() === req.params.movie_id
            ).length > 0
        )
